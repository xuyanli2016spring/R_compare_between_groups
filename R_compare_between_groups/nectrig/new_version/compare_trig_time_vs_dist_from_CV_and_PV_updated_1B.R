# Name: Yanli Xu
# Date: Jan 17th, 2017
# Note: This script is used for comparing the nectrig_dist_from_CV_or_PV_vs_time of the different concentrations in one group and also the difference between different groups, the output is every plot includes all the concentrations in one group. 
...
# make the terminal work under R, then enter command source("compare_trig_time_vs_dist_from_CV_or_PV_by_updated_1B.R"), the script will start working 
...
# compare two groups of experiemtns, then you should put the two groups of experiments("nectrig.csv" files) in two different directories. 

# The following is an example to run this script
# > source("compare_trig_time_vs_dist_from_CV_and_PV_updated_1B.R")
  ...
  for(z in zone)
  {
    for(t in type_dist)
    {
      ...
      for(grp_num in 1:num_of_groups)
      {
        # Read the files
        files <- list.files(path = exp_dirs[grp_num], pattern = "[0-9]+_nectrig.csv", recursive = T)
        plot(c(), c(), type = "l", main = paste("Trig_Time_vs_", t, "_dist_to_", z, "_", exp_group_names[grp_num], sep = ""), xlab = "time", ylab = "trig_dist", xlim = c(0, run_time), ylim = c(y_min, y_max))
        ...
        if(grp_num == 1)
        {
          ...
          if(z == "CV")
          {
            legend((run_time*0.5), (y_max + y_min)*0.6, conc, col=color, cex=1,text.font=1, lty = c(1, 1), bg='lightblue')
          }
          else
          {
