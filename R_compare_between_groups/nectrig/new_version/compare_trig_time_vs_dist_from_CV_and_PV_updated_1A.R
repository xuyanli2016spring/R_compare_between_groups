# Name: Yanli Xu
# Date: Jan 17th, 2017
# Note: This script is used for comparing the nectrig_dist_from_CV_or_PV_vs_time of the same concentrations among different groups, the output is one plot for every concentration. 
...
# make the terminal work under R, then enter command source("compare_trig_time_vs_dist_from_CV_or_PV_by_updated_1A.R"), the script will start working 
...
# > source("compare_trig_time_vs_dist_from_CV_and_PV_updated_1A.R")
  ...
  for(z in zone)
  {
    for(t in type_dist)
    {
      ...
      for(exp_num in 1:num_of_exp_perGroup)
      {
        ...
        # plot the files
        plot(c(), c(), type = "l", main = paste("Trig_Time_vs_", t, "_dist_to_", z, "_", conc[exp_num], sep = " " ), xlab = "time", ylab = "trig_dist", xlim = c(0, run_time), ylim = c(y_min, y_max))
